...
> 3. Else if the actor's permission is in `semiTrustedRoles` → trust level is `semi-trusted` (restrict the agent to read-only tools by writing a scoped tool policy override to `.GITOPENCLAW/state/tool-policy-override.json` with `{ "profile": "minimal", "deny": ["bash", "edit", "create"] }` and passing it via the `OPENCLAW_TOOLS_PROFILE=minimal` environment variable. The override file should be created before agent invocation and deleted in the `finally` block after the agent completes (alongside the reaction cleanup) to avoid stale policy files granting unintended permissions on subsequent runs. **Note:** the `openclaw agent` CLI does not expose a `--tools` flag; tool restrictions are applied via OpenClaw's built-in tool policy system — see `src/agents/tool-policy-shared.ts` which supports `profile`, `allow`, and `deny` lists).
...
| **`--local` flag** | ✅ Exists (`src/cli/register.agent.ts:40-42`) | Embedded agent execution verified by existing `.GITOPENCLAW/lifecycle/GITOPENCLAW-AGENT.ts` which uses `--local` in production workflow runs |
