...
> **Recommended default:** Strategy 2 — "Per-issue concurrency groups" — provides the best balance of safety and throughput for most repositories. Implement it first; fall back to Strategy 1 — "GitHub Actions concurrency group" (global serialisation) — only if cross-issue state conflicts are observed.
...
> 3. Else if the actor's permission is in `semiTrustedRoles` → trust level is `semi-trusted` (restrict the agent to read-only tools by injecting a tool restriction directive into the system prompt context — e.g., prepend `"You are operating in read-only mode. Do not use bash, edit, or create tools."` to the agent's `--message` input, and additionally write a scoped tool policy override to `.GITOPENCLAW/state/tool-policy-override.json` with `{ "profile": "minimal", "deny": ["bash", "edit", "create"] }` for audit purposes. The override file should be created before agent invocation and deleted in the `finally` block after the agent completes (alongside the reaction cleanup) to avoid stale policy files granting unintended permissions on subsequent runs. **Note:** the `openclaw agent` CLI does not expose a `--tools` flag or an `OPENCLAW_TOOLS_PROFILE` environment variable; tool profiles are resolved programmatically via `resolveToolProfilePolicy(profile)` in `src/agents/tool-policy-shared.ts`, which is called internally by the embedded runner with `profile`, `allow`, and `deny` lists. Since the GITOPENCLAW agent invokes `openclaw agent` as a subprocess (not a library call), the most reliable enforcement is system-prompt-level restriction combined with post-run audit of tool calls in `meta.pendingToolCalls` from the `--json` output. Four built-in profiles exist: `minimal`, `coding`, `messaging`, and `full`).
...
>
> In `.GITOPENCLAW/lifecycle/GITOPENCLAW-AGENT.ts`, enforce the `maxTokensPerRun` and `maxToolCallsPerRun` limits. **Note:** the `openclaw agent` CLI does not expose `--max-tokens` or `--max-tool-calls` flags. Instead, use the `--timeout` flag (which the CLI does support, registered at `src/cli/program/register.agent.ts:46-48`) to set `workflowTimeoutMinutes` as a hard time boundary. For token and tool-call limits, implement a post-run enforcement strategy: after the agent completes, parse the JSON output's `meta.agentMeta.usage.total` field for actual tokens consumed. If the total exceeds `maxTokensPerRun`, log a budget violation to `.GITOPENCLAW/state/usage.log` and optionally post a warning comment. Tool-call counts: the `--json` output includes a `meta.pendingToolCalls` array (each entry has `id`, `name`, `arguments`) representing tool calls at the point execution ended. For a complete tool-call count, parse the session transcript JSONL at `.GITOPENCLAW/state/sessions/<id>.jsonl` and count entries with a tool-call role/type. After the agent completes, append a one-line JSON entry to `.GITOPENCLAW/state/usage.log` with: `timestamp`, `issueNumber`, `actor`, `tokensUsed` (from `meta.agentMeta.usage.total`), `tokensInput` (from `meta.agentMeta.usage.input`), `tokensOutput` (from `meta.agentMeta.usage.output`), `cacheRead` (from `meta.agentMeta.usage.cacheRead`), `cacheWrite` (from `meta.agentMeta.usage.cacheWrite`), `durationMs` (from `meta.durationMs`), `stopReason` (from `meta.stopReason`). Set `merge=union` git attribute for `usage.log` in `.GITOPENCLAW/state/.gitattributes`.
>
> Validate in the preflight script (Task 0.1) that if `limits` is present, all values pass schema validation.

---

> **🔭 Milestone — First Light: Issue Processing.**  After Task 0.3 the existing agent pipeline can securely process GitHub Issues end-to-end: settings are validated (0.1), user trust levels are enforced (0.2), and cost/resource boundaries are in place (0.3). The prototype `GITOPENCLAW-AGENT.ts` already receives `issues`/`issue_comment` events, runs the OpenClaw agent, and posts replies — Phase 0 makes that loop production-safe.
...
> 2. Detects if the first line starts with a `/` prefix (e.g., `/config set provider openai`, `/status`, `/help`, `/doctor`, `/channels status`, `/memory search query`, `/sessions list`, `/models list`, `/cron create`, `/plugins install`, `/update`, `/skills list`). **Note:** these map to actual `openclaw` CLI commands. Verify each maps to a real subcommand via `openclaw --help`; the current CLI registers commands in two files — core commands in `src/cli/program/command-registry.ts` and sub-CLI commands in `src/cli/program/register.subclis.ts`. The full command list is: **Core:** `setup`, `onboard`, `configure`, `config`, `doctor`, `dashboard`, `reset`, `uninstall`, `message`, `memory`, `agent`, `agents`, `status`, `health`, `sessions`, `browser`. **Sub-CLIs:** `acp`, `gateway`, `daemon`, `logs`, `system`, `models`, `approvals`, `nodes`, `devices`, `node`, `sandbox`, `tui`, `cron`, `dns`, `docs`, `hooks`, `webhooks`, `qr`, `clawbot`, `pairing`, `plugins`, `channels`, `directory`, `security`, `secrets`, `skills`, `update`, `completion`. Note: there is no `login` command — authentication is handled via `configure` and `onboard`).
...
> {
>   "enforcement": "enforce",
>   "allowedCommands": ["help", "status", "doctor", "config", "configure", "channels", "sessions", "memory", "models", "cron", "plugins", "skills", "update", "agent", "health", "onboard", "nodes"],
...
>    - **Approach B (fragile):** Dynamically import the Commander `program` object from the CLI entry point. OpenClaw's CLI registration files are in `src/cli/program/register.*.ts` (e.g., `register.agent.ts`) and `src/cli/program/command-registry.ts`. This requires resolving ESM imports and TypeScript compilation, which may break across versions. Only use if Approach A is insufficient.
...
>
> **JSON output mapping:** The `openclaw agent --json --local` output (defined by `EmbeddedPiRunResult` in `src/agents/pi-embedded-runner.ts` and serialized via `src/commands/agent/delivery.ts`) provides `meta.durationMs` for duration and `meta.agentMeta.usage` with `{ input, output, cacheRead, cacheWrite, total }` for token counts (all optional numbers — default to `0` if absent). Map these directly to the manifest's `execution` block. The `meta.agentMeta.provider` and `meta.agentMeta.model` fields supply the config section. The `meta.agentMeta.sessionId` provides the session ID. The `meta.stopReason` and `meta.error` (with `kind` enum: `context_overflow`, `compaction_failure`, `role_ordering`, `image_size`, `retry_limit`) fields determine the `outcome` value. `meta.pendingToolCalls` (array of `{ id, name, arguments }`) can be logged for audit. Handle cases where the agent crashes before populating `meta.agentMeta.usage` by defaulting numeric fields to `0` and setting `outcome` to `"error"` — the `--json` output may be incomplete or absent if the process is killed by timeout.
...
|---|---|---|
| **`--tools` CLI flag** (Task 0.2) | Does not exist in `openclaw agent` | Tool profiles resolved programmatically via `resolveToolProfilePolicy(profile)` in `src/agents/tool-policy-shared.ts`. No `OPENCLAW_TOOLS_PROFILE` env var exists. Four built-in profiles: `minimal`, `coding`, `messaging`, `full`. For subprocess invocation, use system-prompt-level restriction + post-run audit |
...
| **`--max-tool-calls` flag** (Task 0.3) | Does not exist in `openclaw agent` | `meta.pendingToolCalls` array available in `--json` output for end-of-run pending calls; for full count, parse session JSONL transcript |
| **`--timeout` flag** (Task 0.3) | ✅ Exists (`src/cli/program/register.agent.ts:46-48`) | Maps directly to `workflowTimeoutMinutes` |
| **`--json` output** (Task 4.1) | ✅ Provides `meta.agentMeta.usage` with `input`, `output`, `cacheRead`, `cacheWrite`, `total`; also `meta.durationMs`, `meta.stopReason`, `meta.error`, `meta.pendingToolCalls`, `meta.agentMeta.provider`, `meta.agentMeta.model`, `meta.agentMeta.sessionId` | Token tracking and run manifest population fully supported |
| **`--local` flag** | ✅ Exists (`src/cli/program/register.agent.ts:40-42`) | Embedded agent execution verified by existing `.GITOPENCLAW/lifecycle/GITOPENCLAW-AGENT.ts` which uses `--local` in production workflow runs |
...
| **GitHub API rate limit** | `GITHUB_TOKEN`: 1000 req/hr/repo; OAuth: 5000 req/hr ([docs](https://docs.github.com/en/rest/rate-limit)) | Document in Task 2.6 polling strategy |
...
Phase 0 (Foundation — Security & Reliability)                     writes → .GITOPENCLAW/ only
  ...
  0.2  Trust-Level Gating                                           ✅ boundary-safe — CORRECTED (system-prompt restriction + post-run audit, not --tools flag or env var)
  0.3  Cost and Resource Boundaries                                 ✅ boundary-safe — CORRECTED (post-run enforcement via --json output + session JSONL parsing)
  ─── 🔭 First Light: Issue Processing (agent can securely process Issues end-to-end) ───
...
*Feasibility-verified on 2026-03-01 against the OpenClaw source tree (`src/cli/program/register.*.ts`, `src/cli/program/command-registry.ts`, `src/commands/agent/delivery.ts`, `src/agents/tool-policy-shared.ts`, `src/agents/pi-embedded-runner.ts`) and GitHub Actions/Pages platform capabilities. All corrections inline.*
